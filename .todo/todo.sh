...
shorthelp()
{
    cat <<-EndHelp
		  ...
		  Actions:
		    ...
		    appendfile|appf SRC ITEM# "TEXT TO APPEND"
		    archive
		    command [ACTIONS]
		    deduplicate
		    del|rm ITEM# [TERM]
...
help()
{
    ...
    cat <<-EndActionsHelp
		  Built-in Actions:
		    ...
		    appendfile SRC ITEM# "TEXT TO APPEND"
		    appf SRC ITEM# "TEXT TO APPEND"
		      Adds TEXT TO APPEND to the end of the task on line ITEM# of file SRC.
		      Quotes optional.
...
"appendfile" | "appf" )
    errmsg="usage: $TODO_SH appendfile SRC ITEM# \"TEXT TO APPEND\""
	shift; srcfile=$1; 

	[ -f "$TODO_DIR/$srcfile" -o -f "$TODO_DIR/$srcfile.txt" ] || die "$srcfile not found"

	if [ -f "$TODO_DIR/$srcfile" ] ; then
		TODO_FILE="$TODO_DIR/$srcfile"
	fi

	if [ -f "$TODO_DIR/$srcfile.txt" ] ; then
		TODO_FILE="$TODO_DIR/$srcfile.txt"
	fi
...
"redo" )
    errmsg="usage: $TODO_SH redo ITEM#[, ITEM#, ITEM#, ...]"
    # shift so we get arguments to the do request
    shift;
    [ "$#" -eq 0 ] && die "$errmsg"
    ...
    for item in ${*//,/ }; do
        ...
        # Check if this item has already been done
        if [ "${todo:0:2}" = "x " ]; then
		   	todo=$(echo $todo | cut -d' ' -f3-)
        	"$TODO_FULL_SH" add $todo
        else
            echo "TODO: $item is not done."
        fi
    done
